...
module.exports = async function handler(req, res) {
  ...
  try {
    const data = await fetchUrl('https://api.argentinadatos.com/v1/finanzas/fci/mercadoDinero/ultimo');
    ...
    data.forEach(fund => {
      ...
      const platform = PLATFORM_MAP.find(p => p.keywords.some(k => nameLower.includes(k)));
